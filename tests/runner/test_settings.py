...
class BaseTests:
    ...
    class TestSettings(unittest.TestCase):
        ...
        def _get_device_specific_settings(self, base_settings):
            setting_map = {
                e_device_settings_type.DeviceFire3SettingsType: "fire3",
                e_device_settings_type.DeviceFire2SettingsType: "cyan",
                e_device_settings_type.DeviceVCAN412SettingsType: "vcan4_12",
                e_device_settings_type.DeviceRADMoon2SettingsType: "radmoon2",
            }
            return getattr(
                base_settings.Settings, setting_map[self.device_settings_type]
            )
        ...
        def test_get_set_settings(self):
            ...
            try:
                ...
                self.assertEqual(
                    base_settings.DeviceSettingType,
                    self.device_settings_type,
                    f"Wrong settings type on {device}",
                )
                ...
                if self.device_type != 5:
                    device_settings.canfd1.FDBaudrate = (
                        ics.BPS1000
                    )  # change CAN1 baudrate
                else:
                    device_settings.opEth1.link_spd = 0x1  # change ETH1 speed
                ics.set_device_settings(device, base_settings)
                time.sleep(sleep_time)
                verify_new_settings = self._get_device_specific_settings(
                    ics.get_device_settings(device)
                )
                if self.device_type != 5:
                    self.assertEqual(
                        verify_new_settings.canfd1.FDBaudrate,
                        ics.BPS1000,
                        f"Failed to set settings on {device}",
                    )
                else:
                    self.assertEqual(
                        verify_new_settings.opEth1.link_spd,
                        0x1,
                        f"Failed to set settings on {device}",
                    )
                ...
                ics.load_default_settings(device)
                time.sleep(sleep_time)
                verify_default_settings = self._get_device_specific_settings(
                    ics.get_device_settings(device)
                )
                if self.device_type != 5:
                    self.assertEqual(
                        ...
                        ics.BPS2000,
                        f"Failed to load default settings on {device}",
                    )
                else:
                    self.assertEqual(
                        verify_default_settings.opEth1.link_spd,
                        0x2,
                        f"Failed to load default settings on {device}",
                    )
            finally:
